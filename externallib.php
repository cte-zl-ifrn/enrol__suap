...
/**
 * SUAP enrolment external functions.
 *
 * @package    enrol_suap
 * @category   external
 * @copyright  2011 Jerome Mouneyrac
 ...
 * @since Moodle 2.2
 */
class enrol_suap_external extends external_api {
    ...
    public static function enrol_users($enrolments) {
        ...
        // Retrieve the suap enrolment plugin.
        $enrol = enrol_get_plugin('suap');
        if (empty($enrol)) {
            throw new moodle_exception('suappluginnotinstalled', 'enrol_suap');
        }
        ...
        foreach ($params['enrolments'] as $enrolment) {
            ...
            // Check that the user has the permission to suap enrol.
            ...
            if (!array_key_exists($enrolment['roleid'], $roles)) {
                ...
                throw new moodle_exception('wsusercannotassign', 'enrol_suap', '', $errorparams);
            }

            // Check suap enrolment plugin instance is enabled/exist.
            $instance = null;
            $enrolinstances = enrol_get_instances($enrolment['courseid'], true);
            foreach ($enrolinstances as $courseenrolinstance) {
              if ($courseenrolinstance->enrol == "suap") {
            ...
            if (empty($instance)) {
              $errorparams = new stdClass();
              $errorparams->courseid = $enrolment['courseid'];
              throw new moodle_exception('wsnoinstance', 'enrol_suap', $errorparams);
            }
            ...
            if (!$enrol->allow_enrol($instance)) {
                ...
                $errorparams->courseid = $enrolment['courseid'];
                $errorparams->userid = $enrolment['userid'];
                throw new moodle_exception('wscannotenrol', 'enrol_suap', '', $errorparams);
            }
    ...
    public static function unenrol_users($enrolments) {
        ...
        foreach ($params['enrolments'] as $enrolment) {
            ...
            $instance = $DB->get_record('enrol', array('courseid' => $enrolment['courseid'], 'enrol' => 'suap'));
            if (!$instance) {
                throw new moodle_exception('wsnoinstance', 'enrol_suap', $enrolment);
            }
            ...
            if (!$enrol->allow_unenrol($instance)) {
                throw new moodle_exception('wscannotunenrol', 'enrol_suap', '', $enrolment);
            }
