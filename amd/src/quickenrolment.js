 ...
 *
 * @module     enrol_suap/quickenrolment
...
const Selectors = {
    ...
    triggerButtons: ".enrolusersbutton.enrol_suap_plugin [type='submit']",
...
const getBodyForContext = contextId => {
    return Fragment.loadFragment('enrol_suap', 'enrol_users_form', contextId, {});
};
...
const showModal = (dynamicTable, contextId) => {
    const pendingPromise = new Pending('enrol_suap/quickenrolment:showModal');

    return ModalFactory.create({
        type: ModalFactory.types.SAVE_CANCEL,
        large: true,
        title: Str.get_string('enrolusers', 'enrol_suap'),
        body: getBodyForContext(contextId),
        buttons: {
            save: Str.get_string('enrolusers', 'enrol_suap'),
        }
    })
        .then(modal => {
        ...
        .then(([modal, body]) => {
            if (body.get(0).querySelector(Selectors.cohortSelector)) {
                return modal.setSaveButtonText(Str.get_string('enroluserscohorts', 'enrol_suap')).then(() => modal);
            }
...
const submitFormAjax = (dynamicTable, modal) => {
    ...
    jQuery.ajax(
        `${Config.wwwroot}/enrol/suap/ajax.php?${form.serialize()}`,
        {
            type: 'GET',
            processData: false,
...
/**
 * Set up quick enrolment for the suap enrolment plugin.
 *
...
export const init = ({ contextid }) => {
    ...
    Prefetch.prefetchStrings('enrol_suap', [
        'enrolusers',
        'enroluserscohorts',
    ]);
