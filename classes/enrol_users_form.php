 ...
 * Enrol users form.
 *
 * Simple form to search for users and add them using a suap enrolment to this course.
 *
 * @package enrol_suap
...
class enrol_suap_enrol_users_form extends moodleform {
    ...
    public function definition() {
        ...
        $options = array(
            'ajax' => 'enrol_suap/form-potential-user-selector',
            'multiple' => true,
            'courseid' => $course->id,
            'enrolid' => $instance->id,
        ...
        $mform->addElement('autocomplete', 'userlist', get_string('selectusers', 'enrol_suap'), array(), $options);
        ...
        if (has_capability('moodle/cohort:manage', $context) || has_capability('moodle/cohort:view', $context)) {
            ...
            if (!empty($availablecohorts)) {
                ...
                $mform->addElement('cohort', 'cohortlist', get_string('selectcohorts', 'enrol_suap'), $options);
            }
        }

        $roles = get_assignable_roles($context, ROLENAME_BOTH);
        $mform->addElement('select', 'roletoassign', get_string('assignrole', 'enrol_suap'), $roles);
