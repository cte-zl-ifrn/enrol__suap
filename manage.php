...
$instance = $DB->get_record('enrol', array('id'=>$enrolid, 'enrol'=>'suap'), '*', MUST_EXIST);
$course = $DB->get_record('course', array('id'=>$instance->courseid), '*', MUST_EXIST);
$context = context_course::instance($course->id, MUST_EXIST);

require_login($course);
$canenrol = has_capability('enrol/suap:enrol', $context);
$canunenrol = has_capability('enrol/suap:unenrol', $context);
...
if (!$canenrol and !$canunenrol) {
    ...
    require_capability('enrol/suap:unenrol', $context);
}
...
if (!$enrol_suap = enrol_get_plugin('suap')) {
    throw new coding_exception('Can not instantiate enrol_suap');
}

$url = new moodle_url('/enrol/suap/manage.php', ['enrolid' => $instance->id]);
$title = get_string('managesuapenrolements', 'enrol_suap');
...
$potentialuserselector = new enrol_suap_potential_participant('addselect', $options);
$currentuserselector = new enrol_suap_current_participant('removeselect', $options);
...
$basemenu[4] = get_string('now', 'enrol_suap') . ' (' . userdate($now, get_string('strftimedatetimeshort')) . ')';
...
if ($canenrol && optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstoassign)) {
        foreach($userstoassign as $adduser) {
            ...
            $enrol_suap->enrol_user($instance, $adduser->id, $roleid, $timestart, $timeend);
        }
...
if ($canunenrol && optional_param('remove', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstounassign)) {
        foreach($userstounassign as $removeuser) {
            $enrol_suap->unenrol_user($instance, $removeuser->id);
        }
                ...
                <td id="buttonscell">
                    <div id="addcontrols">
                        ...
                        <div class="enroloptions">
                            ...
                            <p><label for="menuroleid"><?php print_string('assignrole', 'enrol_suap') ?></label><br />
                    ...
                    <div id="removecontrols">
                        <input class="btn btn-secondary" name="remove" id="remove" <?php echo $removeenabled; ?>
